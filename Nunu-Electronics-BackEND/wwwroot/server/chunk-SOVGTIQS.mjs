  ...
  onFileSelected(event) {
    ...
    if (input?.files?.[0]) {
      ...
      reader.onload = () => {
        ...
        console.log("Image loaded:", this.imageBase64?.substring(0, 100));
      };
      reader.readAsDataURL(file);
    }
  }
  ...
  onSubmit() {
    if (this.productForm.valid && this.imageBase64) {
      ...
      this.productService.addProduct(product).subscribe({
        ...
        error: (err) => {
          alert("Error adding product: " + (err.error || err.message));
        }
      });
    } else {
...
//# sourceMappingURL=chunk-SOVGTIQS.mjs.map
