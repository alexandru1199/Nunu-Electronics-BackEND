...
var AdminAddProductComponent = class AdminAddProductComponent2 {
  ...
  // Metodă pentru selectarea unui fișier de imagine
  onFileSelected(event) {
    const input = event.target;
    if (input?.files?.[0]) {
      const file = input.files[0];
  ...
  // Funcție pentru a declanșa input-ul de fișier
  triggerFileInput() {
    const fileInput = document.querySelector(".file-input");
    if (fileInput) {
      fileInput.click();
  ...
  // Metodă pentru trimiterea formularului
  onSubmit() {
    if (this.productForm.valid && this.imageBase64) {
      const product = {
        ...
        // Adăugăm data curentă pentru createdDate
      };
...
//# sourceMappingURL=chunk-ADXF6HGS.mjs.map
